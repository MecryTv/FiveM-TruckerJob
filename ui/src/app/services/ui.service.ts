...
import { UIMessage } from '../interfaces/UIMessage';
...
export class UiService {
  ...
  constructor() {
    this.messageObservable = fromEvent<MessageEvent<UIMessage>>(
      window,
      'message'
    );
    this.messageObservable.subscribe((event: MessageEvent<UIMessage>) => {
  ...
  async fetchNui<T = any>(
    ...
    const response = await fetch(
      `https://${this.resourceName}/${eventName}`,
      options
    );
  ...
  public dispatchDebugMessages<P>(
    events: UIMessage<P>[],
    timeout = 1000
  ): void {
